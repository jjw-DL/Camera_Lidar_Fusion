...
void show3DObjects(std::vector<BoundingBox> &boundingBoxes, cv::Size worldSize, cv::Size imageSize, bool bWait)
{
    ...
    for(auto it1=boundingBoxes.begin(); it1!=boundingBoxes.end(); ++it1)
    {
        ...
        for (auto it2 = it1->lidarPoints.begin(); it2 != it1->lidarPoints.end(); ++it2)
        {
            ...
            top = top<y ? top : y;  // y方向的最小值
            left = left<x ? left : x; // x方向的最小值
            bottom = bottom>y ? bottom : y; // y方向的最大值
            right = right>x ? right : x; // x方向的最大值
...
void matchBoundingBoxes(std::vector<cv::DMatch> &matches, std::map<int, int> &bbBestMatches, DataFrame &prevFrame, DataFrame &currFrame)
{
    ...
    int pt_counts[p][c] ={}; // 声明一个[p,c]的二维数组
    ...
    for(auto it = matches.begin();it!=matches.end()-1;++it)
    {
        ...
        for(int i = 0 ; i < p; i++) // 循环查找前一帧中哪个boundingbox包含该匹配点
        {
        ...
        for(int i =0 ; i < c; i++) // 循环查找当前帧中哪个boundingbox包含该匹配点
        {
    ...
    // 查找前一帧中第i个boxes和当前帧中第j个boxes匹配（根据match匹配次数评估）
    for(int i = 0 ; i < p ;i++)
    {
        int max_count = 0;
        int id_max = 0;
